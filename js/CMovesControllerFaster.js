...
function CMovesControllerFaster(aMatrix){
    ...
    this.getMovesList = function(iRow, iCol, aBoard, bSimpleKing){
        ...
        switch(szType){
            ...
            case KING:{
                    if(bSimpleKing){
                        ///PREVENT INFINITE STACK LOOP WHEN CHECKING CASTLING 
    ...
    this.getSimpleKingMove = function(iRow, iCol, aBoard){
    ...
    this.getKingMove = function(iRow, iCol, aBoard){
        ...
        if(aKingHistory.length===1 && !s_oGame.isCheck()){
            var szCurPlayerTurnColor = oKing.color;
            var szOpponentColor = s_oBoardStateController.getOtherOpponent(szCurPlayerTurnColor);
            ...
            if(bCastlingRight){
                ...
                var bInMatch = this._isListMatching(aMoves, aForbiddenCellList);
                if(!bInMatch){
    ...
    this._getAllMovesList = function(szPlayerColor, aBoard){
        ...
        for(var i=0; i<aBoard.length; i++){
            for(var j=0; j<aBoard[i].length; j++){
                if(aBoard[i][j].color === szPlayerColor){
                    //aPieces.push(aBoard[i][j]);
                    aPieces.push({row: i, col:j});
                }
            }
        }
        ...
        for(var i=0; i<aPieces.length; i++){
            ...
            aMoves[i] = {list: s_oMovesControllerFaster.getMovesList(oLogicPos.row, oLogicPos.col, aBoard, true), piece: aPieces[i]};
        }
        
        return aMoves;
    };
    
    this._isListMatching = function(aMoves, aListToMatch){
