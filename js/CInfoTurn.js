function CInfoTurn(iX, iY, iType, oParentContainer){
    ...
    this.active = function(bVal){
        if(bVal){
            createjs.Tween.get(_oBg, {loop:true}).to({alpha:0}, 750, createjs.Ease.cubicOut).wait(2000).to({alpha:1}, 750, createjs.Ease.cubicIn);
            createjs.Tween.get(_oBgHighlight, {loop:true}).to({alpha:1}, 750, createjs.Ease.cubicOut).wait(2000).to({alpha:0}, 750, createjs.Ease.cubicIn);
        } else {
            _oBg.alpha = 1;
            _oBgHighlight.alpha = 0;
