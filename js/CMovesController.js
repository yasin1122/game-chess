...
function CMovesController(aMatrix){
    ...
    this.getKingMove = function(iRow, iCol, aBoard){
        ...
        if(aKingHistory.length===1 && !s_oGame.isCheck()){
            ...
            if(bCastlingRight){
                var aCastlingMove = {row: iRow, col:6};
                
                var aMatchList = s_oBoardStateController.checkCastlingBlockFromOpponent(BOARD_SPECIAL_CASTLING_RIGHT, oKing.getColor(), aBoard);
                if(aMatchList.length === 0){
                    aNeighborList.push(aCastlingMove);
                }
            ...
            if(bCastlingLeft){
                var aCastlingMove = {row: iRow, col:2};
                
                var aMatchList = s_oBoardStateController.checkCastlingBlockFromOpponent(BOARD_SPECIAL_CASTLING_LEFT, oKing.getColor(), aBoard);
                if(aMatchList.length === 0){
                    aNeighborList.push(aCastlingMove);
                }
